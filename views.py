...
class CategoryCounter:
    ...
    def show_result(self):
        for cat in self.CATEGORIES_COUNTER:
            if self.CATEGORIES_COUNTER[cat] > 0:
                print(
                    f"W kategorii {cat} przetworzono {self.CATEGORIES_COUNTER[cat]} folderów"
                )
    ...
    def show_processed_folders(self):
        ...
        print("----------------------------------------------")
        print(
            f"{len(self.UNIQUE_NUMBERS)} folderów miało nadane unikalne numery, były to numery: "
            f"{self.UNIQUE_NUMBERS}"
        )
        print("----------------------------------------------\n")
...
class Statistics:
    ...
    def show_statistic(self):
        ...
        unique = repo.get_uniqe_stats(date[0], date[1])
        table = [["KATEGORIA", "ILOŚĆ", "UNIKALNE NUMERY PRODUKTÓW W DANYM MIESIĄCU"]]
        for entry in summary:
            if unique:
                table.append([entry, summary[entry], unique[0][0]])
                unique = None
            else:
