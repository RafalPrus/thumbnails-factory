...
class EntryRepository:
    ...
    def get_by_name(self, category_name):
        ...
        with sqlite3.connect("library.db") as connection:
            ...
            cursor.execute(
                "SELECT `id`, `name` from `category` WHERE `name`=?", (category_name,)
            )
            ...
            for category in counter:
                if counter[category] > 0:
                    ...
                    cursor.execute(
                        ...
                        (category_id, counter[category]),
                    )
            list_of_unique_numbers = [
                element[0] for element in self.get_unique_numbers()
            ]
...
class ReportRepository:
    ...
    def get_summary(self, year: str, month: str):
        ...
        with sqlite3.connect("library.db") as connection:
            cursor = connection.cursor()
            cursor.execute(
                """SELECT category_id, amount FROM entry WHERE strftime('%Y%m', created_at) = (?)""",
                (date,),
            )
            return cursor.fetchall()

    def get_uniqe_stats(self, year: str, month: str):
        date = year + month.zfill(2)
        with sqlite3.connect("library.db") as connection:
            cursor = connection.cursor()
            cursor.execute(
                """SELECT count(unique_number) FROM unique_product_numbers WHERE strftime('%Y%m', created_at) = (?)""",
                (date,),
            )
            return cursor.fetchall()
